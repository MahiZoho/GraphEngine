...
class RedisListIterator {
 public:
  ...
  explicit RedisListIterator(const std::string& list_data)
    ...
    if (length_ > 0) {
      if (cur_byte_ + sizeof(cur_elem_length_) <= num_bytes_) {
        ...
        std::cout << "first elem data corrupted ..." << std::endl;
  ...
  void GetCurrent(Slice* curElem) {
    ...
    if (Done()) {
      std::cout << "Invalid dereferencing in GetCurrent ..." << std::endl;
  ...
  // void GetCurrentWithSizeHeader(Slice* curElem) {
  //   // Ensure we are in a valid state
  //   CheckErrors();

  //   // Ensure that we are not past the last element.
  //   if (Done()) {
  //     std::cout << "Invalid dereferencing in GetCurrentWithHeader ..." << std::endl;
  //     ThrowError("Invalid dereferencing.");
  //   }

  //   // Dereference the element
  //   *curElem = Slice(data_+cur_byte_,
  //                    sizeof(cur_elem_length_)+cur_elem_length_);
  // }
 ...
 private: // Private functions
  ...
  void MoveNext() {
    ...
    if (Done()) {
      std::cout << "Iterate Past Itr... in MoveNext ..." << std::endl;
    ...
    if (cur_byte_ + sizeof(cur_elem_length_) > num_bytes_) {
      std::cout << "Corrupt data element in MoveNext ..." << std::endl;
  ...
  void WriteCurrentElement() {
    ...
    CheckErrors();
    if (Done()) {
      std::cout << "Attempting to write invalid element. ..." << std::endl;
  ...
  void CheckErrors() {
    ...
    if ((cur_elem_ > length_) ||                              // Bad index
      ...
      std::cout << "Check Errors.. corrupt data. ..." << std::endl;
