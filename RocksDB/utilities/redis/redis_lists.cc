...
RedisLists::RedisLists(const std::string& db_path,
  ...
  db_ = db;
  std::cout << " ==== RedisLists Instance Created Successfully ==== " << std::endl;
  std::string propValue;
  db_->GetProperty(Slice("kBaseLevel"), &propValue);
  std::cout << " ==== db prop value ==== " << propValue << std::endl;
}

RedisLists::RedisLists(const std::string& db_path, DB* db, const WriteOptions& write_options, const ReadOptions& read_options)
    : db_name_(db_path),
      put_option_(write_options),
      get_option_(read_options),
      db_(db) {
...
// std::string& RedisLists::getIndexData(const RedisListIterator &it, int32_t index, const bool &header) {
//   // Handle REDIS negative indices (from the end); fast iff Length() takes O(1)
//   std::string result;
//   if (index < 0) {
...
// std::string GetSubString(const RedisListIterator &it, const int &start, const int &end){
//     std::string substring;
//     if(start > -1 && end < Length()){
//       int index = start;
//       while(index <= end){
//         substring = substring+getIndexData(it, index, true);
//         index++;
//       }
//     }
//     return substring;
...
Status RedisLists::getOriginalData(ColumnFamilyHandle* column_family, const std::string& key, std::string &value){
  return db_->Get(get_option_, column_family, key, &value);
}

Status RedisLists::getListData(ColumnFamilyHandle* column_family, const std::string& key, std::vector<std::string> &datavector){
  std::string value;
  Status s = db_->Get(get_option_, column_family, key, &value);
  if(!s.ok()){
    return s;
  }

  Slice elem;
  for (RedisListIterator it(value); !it.Done(); it.Skip()) 
  {
      it.GetCurrent(&elem);
      datavector.push_back(elem.ToString());
  }
  return s;
}
// Print the (list: key) out to stdout. For debugging mostly. Public for now.
void RedisLists::Print(ColumnFamilyHandle* column_family, const std::string& key) {
  // Extract the string data representing the list.
  std::string data;
  db_->Get(get_option_, column_family, key, &data);
...
// // Print the (list: key) out to stdout. For debugging mostly. Public for now.
// void RedisLists::PrintList(const std::string& key) {
//   // Extract the string data representing the list.
//   std::string data;
//   db_->Get(get_option_, key, &data);
...
Status RedisLists::InsertBefore(const std::string& key, const std::string& pivot,
                             const std::string& value) {
  return Insert(key, pivot, value, false);
}

Status RedisLists::InsertAfter(const std::string& key, const std::string& pivot,
                            const std::string& value) {
...
Status RedisLists::PushLeft(ColumnFamilyHandle* column_family, const std::string& key, const std::string& value) {
  // Get the original list data
  // std::cout << "Args to PushLeft... key = " << key << " and value = " << value << std::endl;
  std::string data;
  // Status s = db_->Get(get_option_, column_family, key, &data);
  // if (!s.ok()){
  //   return s;
  // }
  db_->Get(get_option_, column_family, key, &data);
  ...
  // printf("it size = %d\n",it.Size());
  it.Reserve(it.Size() + it.SizeOf(value));
  // printf("it new size reserved = %d\n",it.Size() + it.SizeOf(value));
  it.InsertElement(value);
  // std::cout << "3333" << std::endl;

  // Push the data back to the db and return the length
  // const char* tt = it.WriteResult().data();
  // printf("data to be persisted = %s\n", tt);
  return db_->Put(put_option_, column_family, key, it.WriteResult());
...
//   : throws RedisListException
Status RedisLists::PushRight(const std::string& key, const std::string& value) {
  // Get the original list data
  std::string data;
  db_->Get(get_option_, key, &data);
  ...
  return db_->Put(put_option_, key, it.WriteResult());
  //return it.Length();
}
...
//   : throws RedisListException
Status RedisLists::Insert(const std::string& key, const std::string& pivot,
