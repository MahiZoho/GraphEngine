...
struct rocksdb_column_family_handle_list_t  { 
  ColumnFamilyHandle** rep;
  int cflen; //count of column families ....
  int size; //size of array...
};
// struct rocksdb_column_family_handle_struct_array_t  { 
//   rocksdb_column_family_handle_t** rep;
//   int cflen; //count of column families ....
//   int size; //size of array...
// };
// struct rocksdb_data_t {
//   char **data;
//   int size;
// };
struct rocksdb_redis_lists_t  { RedisLists* rep; };
...
rocksdb_t* rocksdb_open_column_families_default_options(
    const rocksdb_options_t* db_options,
    const char* name,
    int num_column_families,
  ...
  for (int i = 0; i < num_column_families; i++) {
    column_families.push_back(ColumnFamilyDescriptor(
        std::string(column_family_names[i]),
        ColumnFamilyOptions(db_options->rep)));
  }
...
rocksdb_t* rocksdb_open_column_families_default_options_zlabs(
    const rocksdb_options_t* db_options,
    const char* name,
    int num_column_families,
    char** column_family_names,
    rocksdb_column_family_handle_struct_array_t **column_family_handle_list,
    char** errptr) {
  std::vector<ColumnFamilyDescriptor> column_families;
  for (int i = 0; i < num_column_families; i++) {
    column_families.push_back(ColumnFamilyDescriptor(std::string(column_family_names[i]), ColumnFamilyOptions(db_options->rep)));
  }
  ...
  try{
    column_family_handle_list[0] = new rocksdb_column_family_handle_struct_array_t;
    rocksdb_column_family_handle_t **&cf_handle_struct_array = column_family_handle_list[0]->rep;
    cf_handle_struct_array = (rocksdb_column_family_handle_t**)malloc(num_column_families * 2 * (sizeof(rocksdb_column_family_handle_t*)));

    for (size_t i = 0; i < handles.size(); i++) {
      rocksdb_column_family_handle_t* c_handle = new rocksdb_column_family_handle_t;
      c_handle->rep = handles[i];
      cf_handle_struct_array[i] = c_handle;
    }
    column_family_handle_list[0]->cflen = num_column_families;
    column_family_handle_list[0]->size = num_column_families * 2;
  }catch(std::exception& e){
    Status s = Status::ZlabsError("Error while populating ColumnFamilyList!\n");
...
void rocksdb_column_family_handle_list_destroy(rocksdb_column_family_handle_struct_array_t *columnFamilyHandleList){
  ...
  delete columnFamilyHandleList;
}

rocksdb_column_family_handle_t* rocksdb_GetColumnFamilyHandle(rocksdb_column_family_handle_struct_array_t *columnFamilyHandleList, const char *column_family_name){
    int i;
    bool found = false;
    rocksdb_column_family_handle_t *handle;

    rocksdb_column_family_handle_t **&cf_handle_struct_array = columnFamilyHandleList->rep;
    int cflen = columnFamilyHandleList->cflen;

    for(i=0; i<cflen; i++){
      handle = cf_handle_struct_array[i];
      const char *name = (handle->rep->GetName()).c_str();
...
char** rocksdb_list_column_families(
  ...
  if(SaveError(errptr, DB::ListColumnFamilies(DBOptions(options->rep), std::string(name), &fams))){
      return nullptr;
  }
...
rocksdb_column_family_handle_t* rocksdb_create_column_family_zlabs(
    rocksdb_t* db,
    const rocksdb_options_t* column_family_options,
  ...
  try{
    rocksdb_column_family_handle_t **&cf_handle_struct_array = columnFamilyHandleList[0]->rep;
    int handlecount = columnFamilyHandleList[0]->cflen;
    
    if (handlecount >= (int)((columnFamilyHandleList[0]->size * 3)/4)) {
        int newsize = columnFamilyHandleList[0]->size * 2;
        cf_handle_struct_array = (rocksdb_column_family_handle_t**)(realloc(cf_handle_struct_array, newsize * (sizeof(rocksdb_column_family_handle_t*))));
        columnFamilyHandleList[0]->size = newsize;
    }
    
    cf_handle_struct_array[handlecount] = handle;
    columnFamilyHandleList[0]->cflen = handlecount+1;
  }catch(std::exception& e){
    Status s = Status::ZlabsError("Error while updating ColumnFamilyList with NEW ColumnFamily!\n");
    SaveError(errptr, s);
    return nullptr;
  }
...
// rocksdb_column_family_handle_t* rocksdb_getColumnFamilyHandle(
//     rocksdb_t* db, 
//     const rocksdb_options_t* column_family_options, 
//     const char* column_family_name, 
//     char** errptr){
//       ColumnFamilyHandle* cf = db->rep->GetColumnFamilyHandleByName(std::string(column_family_name));
//       if(cf != nullptr){
//         rocksdb_column_family_handle_t* result = new rocksdb_column_family_handle_t;
//         result->rep = cf;
//         //db->NewThreadStatusCfInfo(cfd);
//         return result;
//       } else {
//           return rocksdb_create_column_family(db, column_family_options, column_family_name, errptr);
//       }
// }

rocksdb_redis_lists_t* rocksdb_redis_lists_create(const char* name, rocksdb_t* db, const rocksdb_writeoptions_t* writeoptions, const rocksdb_readoptions_t* readoptions) {
  RedisLists *redis = new RedisLists(std::string(name), db->rep, writeoptions->rep, readoptions->rep);
  rocksdb_redis_lists_t* result = new rocksdb_redis_lists_t;
  result->rep = redis;
  return result;
}

void rocksdb_redis_lists_push_left(
    rocksdb_redis_lists_t* redisLists,
    rocksdb_column_family_handle_t* column_family_handle,
    const char* key,
    const char* val,
    char** errptr) {
      // printf("In C.CC Program...inside rocksdb_redis_lists_push_left function....key = %s and value = %s\n", key,val);
      //redisLists->rep->PushLeftStatus(db->rep, std::string(key), std::string(val));
      SaveError(errptr, redisLists->rep->PushLeft(column_family_handle->rep, std::string(key), std::string(val)));
      return;
}

// int rocksdb_redis_lists_get_data(rocksdb_redis_lists_t* redisLists,
//     rocksdb_column_family_handle_t* column_family_handle,
//     const char* key, 
//     char** data, 
//     char** errptr){
...
//     redisLists->rep->getListData(column_family_handle->rep, std::string(key), datavector);

//     //Iterating dataVector and populating into array of strings...
//       int i =0;
//       datacount = static_cast<int>(datavector.size());
//       data = static_cast<char**>(realloc(data, datacount * sizeof(char*)));
//       std::cout << "Printing data vector...." << std::endl;
//       for (std::vector<std::string>::iterator it = datavector.begin() ; it != datavector.end(); ++it){
//         std::cout << ' ' << *it << std::endl;
//         data[i] = strdup(it->c_str());
//         i++;
//       }
//       return datacount;
// }

rocksdb_data_t* rocksdb_redis_lists_get_data(rocksdb_redis_lists_t* redisLists,
    rocksdb_column_family_handle_t* column_family_handle,
    const char* key, 
    char** errptr){

    std::vector<std::string> datavector;
    if(SaveError(errptr, redisLists->rep->getListData(column_family_handle->rep, std::string(key), datavector))){
      return nullptr;
    }
    ...
    //Iterating dataVector and populating into array of strings...
      ...
      char **data = static_cast<char**>(malloc(datacount * sizeof(char*)));
      for (std::vector<std::string>::iterator it = datavector.begin() ; it != datavector.end(); ++it){
        data[i] = strdup(it->c_str());
        i++;
      }
      rocksdb_data_t *rocksdb_data = new rocksdb_data_t;
      rocksdb_data->data = data;
      rocksdb_data->size = datacount;
      return rocksdb_data;
}

void rocksdb_destroy_data(rocksdb_data_t *listdata){
  int i;
  char **data = listdata->data;
...
// int rocksdb_redis_lists_get_common_data(rocksdb_redis_lists_t* redisLists,
//     rocksdb_column_family_handle_t* column_family_handle,
//     const char* key1, 
//     const char* key2, 
//     char** commonData, 
...
//       std::string data2;
//       int commondatacount;
//       std::vector<std::string> commondatavector;

//       //Get data...
//       redisLists->rep->getOriginalData(column_family_handle->rep, std::string(key1), data1);
//       redisLists->rep->getOriginalData(column_family_handle->rep, std::string(key2), data2);

//       //Getting Common Data...
//       RedisLists::getCommonData(data1, data2, commondatavector);

//       //Iterating CommonData Vector and populating into array of strings...
//       int i =0;
//       commondatacount = static_cast<int>(commondatavector.size());
//       commonData = static_cast<char**>(realloc(commonData, commondatacount * sizeof(char*)));
//       std::cout << "Printing common data vector...." << std::endl;
//       for (std::vector<std::string>::iterator it = commondatavector.begin() ; it != commondatavector.end(); ++it){
//         std::cout << ' ' << *it << std::endl;
//         commonData[i] = strdup(it->c_str());
//         i++;
//       }
//       return commondatacount;
// }

rocksdb_data_t* rocksdb_redis_lists_get_common_data(rocksdb_redis_lists_t* redisLists,
    rocksdb_column_family_handle_t* column_family_handle,
    const char* key1, 
    const char* key2, 
    char** errptr){
      ...
      //Get data...
      Status s = redisLists->rep->getOriginalData(column_family_handle->rep, std::string(key1), data1);
      if(s.ok()){
        s = redisLists->rep->getOriginalData(column_family_handle->rep, std::string(key2), data2);
        if(s.ok()){
          s = RedisLists::getCommonData(data1, data2, commondatavector);
          if(s.ok()){
            //Iterating CommonData Vector and populating into array of strings...
            int i =0;
            commondatacount = static_cast<int>(commondatavector.size());
            char **commonData = static_cast<char**>(malloc(commondatacount * sizeof(char*)));
            std::cout << "In C.CC .. Printing common data vector...." << std::endl;
            for (std::vector<std::string>::iterator it = commondatavector.begin() ; it != commondatavector.end(); ++it){
              std::cout << ' ' << *it << std::endl;
              commonData[i] = strdup(it->c_str());
              i++;
            ...
            rocksdb_data_t *rocksdb_data = new rocksdb_data_t;
            rocksdb_data->data = commonData;
            rocksdb_data->size = commondatacount;
            return rocksdb_data;
          }
        }
      }
...
void rocksdb_redis_lists_print(rocksdb_redis_lists_t* redisLists, rocksdb_column_family_handle_t* column_family_handle, const char* key){
  redisLists->rep->Print(column_family_handle->rep, std::string(key));
}

void rocksdb_redis_lists_close(rocksdb_redis_lists_t* redisLists) {
  delete redisLists->rep;;
  delete redisLists;
  redisLists->rep = nullptr;
  redisLists = nullptr;
}
